...
def join_group(group_id):
    ...
    group = db.engine.execute("SELECT max_members FROM groups_table WHERE id = %s", (group_id,)).fetchone()
    count = db.engine.execute("SELECT COUNT(*) FROM memberships WHERE group_id = %s", (group_id,)).fetchone()[0]
    if count >= group[0]:
        return jsonify({"error": "Group is full"}), 400
    existing = db.engine.execute("SELECT id FROM memberships WHERE user_id=%s AND group_id=%s", (user_id, group_id)).fetchone()
    if existing:
        return jsonify({"error": "Already a member"}), 400
    db.engine.execute("INSERT INTO memberships (user_id, group_id) VALUES (%s,%s)", (user_id, group_id))
...
def leave_group(group_id):
    user_id = get_jwt_identity()
    db.engine.execute("DELETE FROM memberships WHERE user_id=%s AND group_id=%s", (user_id, group_id))
